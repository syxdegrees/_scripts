...
def main():
    ...
    if args.item_id:
        ...
        search_row = cache_lookup_search(
            config, normalized, args.domain, args.ttl_days
        )
        if search_row:
        ...
        if len(organic) < args.max_items:
            seen_ids = {str(item.get("product_id")) for item in organic if item.get("product_id")}
            ...
            while len(organic) < args.max_items and extra_pages < 10:
                try:
                    page_data = search_home_depot(
                        serpapi_key, args.search_phrase,
                        domain=args.domain, page=next_page
                    )
                    items_fetched += 1
                    new_results = page_data.get("products", [])
                    if not new_results:
                        break
                    added = 0
                    for item in new_results:
                        item_id = str(item.get("product_id")) if item.get("product_id") else None

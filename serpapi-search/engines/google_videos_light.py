...
def build_params(search_phrase, country, language, pages):
    return {
...
def run(api_key, supabase_config, search_phrase, country, language,
    ...
    call_params = build_params(search_phrase, country, language, pages)
    ...
    for page_num in range(1, pages + 1):
        ...
        if page_num == 1:
            combined = response
        elif "video_results" in response:
            combined["video_results"] = (
                combined.get("video_results", []) + response["video_results"]
            )

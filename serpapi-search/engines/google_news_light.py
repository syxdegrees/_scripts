...
TABLE = "serpapi_google_news_light_cache"

SECTION_COLS = {"search_information", "news_results", "serpapi_pagination"}
...
def run(api_key, supabase_config, search_phrase, country, language,
    ...
    call_params = build_params(search_phrase, country, language, location, pages)
    ...
    for page_num in range(1, pages + 1):
        ...
        if page_num == 1:
            combined = response
        elif "news_results" in response:
            combined["news_results"] = (
                combined.get("news_results", []) + response["news_results"]
            )

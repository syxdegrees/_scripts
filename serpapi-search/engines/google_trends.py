...
TABLE = "serpapi_google_trends_cache"
...
def build_params(search_phrase, data_type="TIMESERIES", tz=None):
    p = {
...
def run(api_key, supabase_config, search_phrase, country, language,
    ...
    # country, language, location, pages not used by this engine
    data_type = engine_params.get("data_type", "TIMESERIES")
    tz = engine_params.get("tz")
    call_params = build_params(search_phrase, data_type, tz)

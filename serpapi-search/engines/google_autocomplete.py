...
TABLE = "serpapi_google_autocomplete_cache"

SECTION_COLS = {"suggestions", "verbatim_relevance"}
_STRIP_KEYS = {"search_metadata", "search_parameters", "search_information"}


def build_params(search_phrase, country, language):
    return {
...
def run(api_key, supabase_config, search_phrase, country, language,
    ...
    # Autocomplete: no location, no pagination
    call_params = build_params(search_phrase, country, language)

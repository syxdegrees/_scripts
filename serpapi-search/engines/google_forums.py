...
TABLE = "serpapi_google_forums_cache"

# Sections from live API refresh: search_information, organic_results, related_searches, serpapi_pagination
SECTION_COLS = {"search_information", "organic_results", "related_searches", "serpapi_pagination"}
_STRIP_KEYS = {"search_metadata", "search_parameters"}


def build_params(search_phrase, country, language, location, pages,
                 nfpr=0, filter=1, period_unit=None, period_value=1,
    ...
    if location:
        p["location"] = location
    if period_unit:
        p["period_unit"] = period_unit
        p["period_value"] = period_value

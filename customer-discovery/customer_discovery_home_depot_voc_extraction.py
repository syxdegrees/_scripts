#!/usr/bin/env python3
"""
customer_discovery_home_depot_voc_extraction.py
Searches Home Depot for a run's discovery phrase, fetches product reviews,
and saves to home_depot_products, home_depot_reviews, and voc_content.

Dependencies: pip install requests

Usage:
    python customer_discovery_home_depot_voc_extraction.py --run_id <uuid>
"""
...
def search_home_depot(serpapi_key, phrase):
    resp = requests.get(
        SERPAPI_URL,
        params={'engine': 'home_depot', 'q': phrase, 'api_key': serpapi_key},
        timeout=30,
    )
    if not resp.ok:
        raise RuntimeError(f"SerpAPI home_depot error {resp.status_code}: {resp.text}")
...
def fetch_reviews(serpapi_key, product_id):
    ...
    if not resp.ok:
        raise RuntimeError(f"SerpAPI home_depot_product error {resp.status_code}: {resp.text}")
    return resp.json().get('reviews', [])


def main():
    parser = argparse.ArgumentParser(description='Home Depot VOC Extraction')
    ...
    try:
        results = search_home_depot(serpapi_key, phrase)
    except Exception as e:
        print(f"ERROR: Home Depot search failed: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    for result in results:
        ...
        try:
            hd_product_id = supabase_insert(supabase_url, secret_key, 'home_depot_products', {
                'run_id': run_id,
        ...
        for review in review_list:
            ...
            try:
                review_id = supabase_insert(supabase_url, secret_key, 'home_depot_reviews', {
                    'home_depot_product_id': hd_product_id,
                    'run_id': run_id,
                ...
                supabase_insert(supabase_url, secret_key, 'voc_content', {
                    ...
                    'home_depot_review_id': review_id,
                })
                voc_saved += 1
            except Exception as e:
